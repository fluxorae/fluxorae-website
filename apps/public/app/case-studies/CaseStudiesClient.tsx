...
const enterpriseStories: Story[] = [
  {
    ...
    slug: '/case-studies',
    tags: ['Logistics', 'Automation', 'Data & Analytics'],
  },
  {
    ...
    slug: '/case-studies',
    tags: ['Finance', 'SaaS', 'Generative AI', 'Data & Analytics'],
  },
  {
    title: 'Clinical AI Command Center',
    partner: 'Healthcare Systems',
    problem: 'Reactive care workflows caused preventable escalations and staffing overload.',
    solution: 'Predictive patient scoring and automation playbooks prioritized admissions, staffing, and discharge orchestration.',
    impact: ['-18% escalations', '+32% triage accuracy', '6h faster bed turnarounds'],
    slug: '/case-studies',
    tags: ['Healthcare', 'Automation', 'Data & Analytics'],
  },
  {
    title: 'Predictive Quality for Precision Manufacturing',
    partner: 'Manufacturing Operations',
    problem: 'Manual inspections delayed shipments and generated avoidable scrap.',
    solution: 'Sensor fusion models flag anomalies in real time and trigger AI operators to adjust systems before defects appear.',
    impact: ['-22% scrap losses', '+12% uptime', 'Inspection cycle -40%'],
    slug: '/case-studies',
    tags: ['Manufacturing', 'Automation', 'Data & Analytics'],
  },
]
...
const startupStories = [
  {
    ...
    tags: ['Retail', 'Automation', 'Data & Analytics'],
  },
  {
...
const filterOptions = [FILTER_ALL, ...filterTags]

export default function CaseStudiesClient() {
  const [activeFilter, setActiveFilter] = useState(FILTER_ALL)

  const filteredEnterpriseStories = useMemo(() => {
    if (activeFilter === FILTER_ALL) {
      return enterpriseStories
    }

    return enterpriseStories.filter((story) => story.tags.includes(activeFilter))
  }, [activeFilter])

  const filteredStartupStories = useMemo(() => {
    if (activeFilter === FILTER_ALL) {
      return startupStories
    }

    return startupStories.filter((story) => story.tags.includes(activeFilter))
  }, [activeFilter])

  const getFilterButtonClass = (tag: string) =>
    `px-3 py-1 rounded-full border uppercase tracking-[0.4em] text-xs transition ${
      activeFilter === tag
    ...
    <main className="bg-primary text-secondary">
      <div className="container-custom py-16 space-y-12">
        ...
        <section className="space-y-4">
          ...
          <p className="text-sm text-gray-500">
            {activeFilter === FILTER_ALL
              ...
              : `Showing ${activeFilter} stories. Filter again or reset to see every narrative.`}
          </p>
        </section>
        ...
        <section className="space-y-8">
          ...
          {filteredEnterpriseStories.length === 0 ? (
            <p className="text-sm text-gray-500">No enterprise stories match the selected filter yet.</p>
          ) : (
            <div className="grid gap-6 md:grid-cols-2">
              {filteredEnterpriseStories.map((story) => (
        ...
        <section className="space-y-8">
          ...
          {filteredStartupStories.length === 0 ? (
            <p className="text-sm text-gray-500">No startup stories match the selected filter yet.</p>
          ) : (
            <div className="grid gap-6 md:grid-cols-2">
              {filteredStartupStories.map((story) => (
                <article key={story.title} className="glass-panel rounded-3xl border border-white/10 p-6 flex flex-col h-full">
                  <div className="text-xs uppercase tracking-[0.4em] text-gray-400 mb-2">{story.partner}</div>
                  <h3 className="text-2xl text-white font-semibold mb-3">{story.title}</h3>
                  <div className="space-y-3 text-sm text-gray-300 flex-grow">
